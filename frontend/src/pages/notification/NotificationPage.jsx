...
const NotificationPage = () => {
  ...
  return (
    ...
    <div className='dropdown '>
						...
						>
							<li className='cursor-pointer '>
								<a onClick={handleDeleteNotications}>Delete all notifications</a>
							</li>
						</ul>
					</div>
    </div>
    {isLoading && (<div className='flex text-3xl flex-col justify-center h-screen items-center'>Loading...<span><LoadingSpinner/></span></div>)}
   ...
   <div className='flex gap-2 items-center'>
    ...
    <img src={notification.from.profileImg} alt="ProfileImage" className="h-10 w-10" />
    </Link>
    </div>
    <div>
     <h1><Link to={`/profile/${notification.from.username}`} className='text-primary'>@{notification.from.username}</Link> {notification.type === "follow" ? "followed you." : "liked your post."}</h1>
     </div>
     </div>
